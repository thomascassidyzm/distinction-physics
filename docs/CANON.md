...
[ASSEMBLER, 2026-08-04, corrected 2026-08-05: partly closed. Tom ruled the project should be renamed and gave the reason — it is not hard physics; it is the metaphysics, logic and ethics of being an OLU. The first pass shortened the wrapper to **PRIMITIVE**; Tom corrected that on 2026-08-05 and the name is **Distinction as Primitive** everywhere, project and treatise alike. The wrapper inside this repo (README, `TREATISE_ARCHITECTURE.md`, `package.json` as the slug `distinction-as-primitive`) now says so. His articulation is quoted verbatim in `NAMING.md`, along with the correction. Still open: the repo slug, the directory, the Vercel URL, and the external citations in CE and `tomcassidy-site` — costed in `docs/rename-blast-radius.md`, not executed.]
...
- `docs/canonical/least-time-learning.md` — **Least-Time Learning, the canonical statement** (ruled by Tom 2026-08-24: name ratified, trinity on the front page, publish now). The single node CE, Zenjin and SSi reference rather than restate; its detail is `src/content/treatise/module-4-learning/section-4.13.ts`.
- `docs/canonical/scaling-is-the-primitive.md` — **Scaling is the primitive, position is an agreement** (ruled by Tom 2026-08-29: comparison is a concept, the numeral and the number line are facts; addition is a scaling by one-plus-a-ratio; wide rungs carry no numerals). The single node the maths re-level and the Zenjin graph work reference rather than restate; the eleven-relation unification is stated there and marked NOT YET CASHED, pending job #203.
