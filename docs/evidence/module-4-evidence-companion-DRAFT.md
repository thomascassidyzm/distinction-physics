...
5. **Adherence is the binding constraint, so shame is off-trajectory by definition.** The energy budget (Axiom 2) is spent by a motivated human or not at all. Any mechanic that converts a missed session into guilt raises the energy cost of returning — action spent against the trajectory. [INFERRED — the treatise derives the budget constraint; reading motivational design as part of the cost functional is the extension this companion registers.]
   ...
   [WATSON — second addition, 2026-08-03; the observation is Tom's, the formalisation mine. REVISED same evening after Tom's correction.] The limit case of prediction 5 is the dog. People love dogs because a dog's affection is, in Tom's words, "the easiest, the cheapest kind of love humans can get" — least action applied to relationship — and the mechanism of the cheapness is that the dog keeps no ledger of your absence, only an ever-richer model of your presence. My first formalisation claimed absence could be made *unrepresentable*; Tom corrected it — telemetry necessarily contains timestamps, so absence is always derivable, and claiming structural impossibility is disingenuous. The corrected law is **asymmetric surfacing with the Sinon property**: (a) absence may be derived (insights require it) but no code path may run from derived absence to any learner-facing output, and absence may never enter the reward function as a penalty; (b) the reward mechanics must be un-gameable *even by the operators* — publish the principle-space (rewards attach only to advocated behaviours: total attention-time, shortest wall-clock path to target, direction of improvement — opaque yet justifiable), while the instance coefficients are sealed, e.g. derived per-learner from a secret seed and held in no human-readable form, so that no learner, employee, or future growth incentive can state or optimise any individual's formula. Like Sinon, the system cannot betray under interrogation what it never knew. Auditable tests, revised: no code path from absence-derivation to a learner surface; no penalty weight on absence; no human-readable instance of the reward coefficients anywhere. Boundary condition against the neighbouring dark pattern: unpredictable reward is also the casino's mechanism — this stays clean only while rewards carry no scarcity and no loss (a message, a skin, a welcome), inputs are only advocated behaviours, and no punishment channel exists. Opaque-and-trivial-stakes is delight; opaque-and-high-stakes is a fruit machine.
...
- **The two-milestone curve.** SSi's operational model is two milestones: **~30 hours to conversational entry** — it starts getting fun, you get into conversations, listening work opens up — and **~75–100 hours to sustained unscripted conversation**. The second milestone is theorised in the treatise and confirmed by the sprints and Croatia. The first — the 30-hour milestone — is **empirical only: it is SSi action-research calibration and is NOT in the treatise.** It is registered here as a datum Module 4 should eventually account for (plausibly the point where enough of the network has automatized that maintenance cost stops dominating — [INFERRED]), not as something the treatise predicts.
  ...
  **Further public record, same design, four languages.** Beyond the Welsh and Croatian cases above, three more publicly timestamped items sit on the same evidence base:
  ...
  - **The Irish sprint** — likewise the Irish sprint probe above, publicly recorded: an interview conducted in Irish on RTÉ Raidió na Gaeltachta: https://www.rte.ie/radio/rnag/clips/22557598/ (confirmed by Tom, 2026-08-03)
