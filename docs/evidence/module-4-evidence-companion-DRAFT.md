...
5. **Adherence is the binding constraint, so shame is off-trajectory by definition.** The energy budget (Axiom 2) is spent by a motivated human or not at all. Any mechanic that converts a missed session into guilt raises the energy cost of returning — action spent against the trajectory. [INFERRED — the treatise derives the budget constraint; reading motivational design as part of the cost functional is the extension this companion registers.]
   ...
   [WATSON — second addition, 2026-08-03; the observation is Tom's, the formalisation mine.] The limit case of prediction 5 is the dog. People love dogs because a dog's affection is, in Tom's words, "the easiest, the cheapest kind of love humans can get" — least action applied to relationship. And the *mechanism* of that cheapness is structural: the dog keeps no ledger. Its memory has no representation of your absence — only an ever-richer model of your presence. The forgiveness is not behavioural generosity; it is schema. This yields a design law for any adherence-bound system: **asymmetric memory — total recall of the learner's becoming, structural amnesia for their absence.** A system whose data model can represent "days missed" will eventually surface it (the guilt email is a query, and queries get written); a system that cannot represent absence is *architecturally incapable* of reproach. The auditable test: inspect the schema — if "streak," "days since," or any attendance ledger exists as a storable quantity, prediction 5 will eventually be violated in production regardless of the copywriting. Conditional delight must be unrepresentable, not merely unexpressed.
