...
**G2 — Per-instant maintenance cost is NOT what Landauer prices; retention is a timescale, not a category.** `[was an unsound step; now repaired]`
The previous proof of Proposition 1.1 (treatise §1.1) read "each such bit costs at least $D_{\min}$ **to maintain**." Landauer does not say that: what Landauer prices is the *write–reset cycle* (P4), not storage per unit time. The proofs in §§1.1–1.2 have been repaired to route through acquisition-cycle accounting, which is sound.

An earlier draft of this gap went further and proposed demoting the Dynamism principle ("distinctions require continuous maintenance") as "false for equilibrium-stable media." **Retracted — owner correction, 2026-07-03.** There is no such thing as a stable medium, only stability over a timescale. Any record is held in a potential well with finite barrier $\Delta E$, and at $T > 0$ it flips spontaneously at the Kramers/Arrhenius rate $\Gamma \sim \Gamma_0 e^{-\Delta E / k_B T} > 0$ `[IMPORTED — statistical mechanics]`, giving a finite expected retention time. This is exactly the shape the treatise already carries in §1.8's decay theorem, $\tau(E) = \tau_0 e^{E/k_B T}$: energy investment buys exponentially longer retention, never infinite retention. Driving $\Gamma \to 0$ requires $\Delta E \to \infty$ (unbounded energy to erect the barrier — excluded by A2) or $T \to 0$ (excluded by L2 — the same closure as G3).

So the rigorous form of Dynamism is *timescale-relative*, and it strengthens rather than demotes: **no distinction is maintained indefinitely without expenditure.** Storage beyond the retention time requires error correction, and each corrected error is an erasure paying the Landauer floor. The per-unit-time cost can be made exponentially small (that is what "stable" means) but never zero, and the prepayment — barrier height — is itself charged against the budget. Status: the finite-retention claim is `[DERIVED]` given imported Kramers escape + A2/A3/L2; the specific exponential decay form is `[IMPORTED]`. The effective-discreteness theorem itself still does not depend on Dynamism; but Dynamism is not a casualty of this tightening — it is another instance of the same ledger.
...
| Universal Dynamism (timescale-relative: no indefinite maintenance without expenditure) | finite-retention claim `[DERIVED]` given imported Kramers escape + A2/A3/L2; decay form `[IMPORTED]` — per revised G2 (owner correction 2026-07-03) |
